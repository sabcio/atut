...
Deface::Override.new(:virtual_path => %q{spree/checkout/_confirm},
  ...
  <div id="order-items" style="padding:0 30px 30px 0">
    ...
    <% @order.line_items.each do |line_item| %>
      <li>
        <%= line_item.quantity %> <%= line_item.variant.product.name %> za
        <%= number_to_currency (line_item.price * line_item.quantity)-%>
      </li>
    <% end %>
    </ul>
    <%= link_to "edytuj", cart_path %>
  </div>
  ...
  <% {"wysyłka" => "bill", "płatność" => "ship"}.each do |key, addr_type| %>
    ...
    <div id="order-#{addr_type}_address" style="float:left; padding:30px 30px 30px 0">
      ...
      <%= link_to "edytuj", checkout_state_path('address') %>
    </div>
  <% end %>
    ...
    <div id="order-payment" style="padding:30px 30px 30px 0; clear:both">
      ...
      <%= link_to "edytuj", checkout_state_path('payment') %>
    </div>
  <div>
  ...
  <div class="submit">
    ...
    <% else %>
      <button class="blue" type="submit">Złóż zamówienie</button>
    <% end %>
  </div>
  </div>
</div>})
